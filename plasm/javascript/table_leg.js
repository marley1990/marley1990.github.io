function drawTable_leg(){
  var button = document.getElementById("object");
  button.setAttribute("onclick","viewTable_leg()");
  var dec = document.getElementById("decomposition");
  dec.setAttribute("onclick","decTable_leg()");
  alert("Click on view object button to see Table_leg")
}

function viewTable_leg(){
 var plasm = document.getElementById("plasm")
      if(plasm == null){
  var message = 'Do you want see the table_leg?'
...
function Animate() {
  var message = 'Do you want to activate the movement of the table_leg?'

  var choice = confirm(message)

  if (choice == true) {
    setInterval(function () {
    table_leg.rotate([0,1], PI/45)/*.rotate([1,2],PI/45).rotate([0,2],PI/45)*/;
    }, 100);
  }
...
function decTable_leg(){
  var message = 'Do you want see the decomposition of table_leg?'
...
var base = T([1])([6-r])(STRUCT([base1,T([1,2])([0.025,h+0.3])(base2)]))
...
var table_legD = white(STRUCT([make_leg(l,r,h),T([2])([0.5])(create_plane(y_plane,z_plane,x_plane)),base,T([0])([-0.5])(behind)]))
